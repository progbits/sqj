...
func TestIdentifiers(t *testing.T) {
	for _, testCase := range identifierTestCases {
		scanner := NewScanner([]byte(testCase.statement))
		tokens := scanAll(scanner)
		checkEquality(t, tokens, testCase.expected)
	}
}
...
func TestRealNumbers(t *testing.T) {
	for _, testCase := range realNumberTestCases {
		scanner := NewScanner([]byte(testCase.statement))
		tokens := scanAll(scanner)
		checkEquality(t, tokens, testCase.expected)
	}
...
func TestIntegers(t *testing.T) {
	for _, testCase := range integerTestCases {
		scanner := NewScanner([]byte(testCase.statement))
		tokens := scanAll(scanner)
		checkEquality(t, tokens, testCase.expected)
	}
...
func TestKeywords(t *testing.T) {
	for _, testCase := range keywordTestCases {
		scanner := NewScanner([]byte(testCase.statement))
		tokens := scanAll(scanner)
		checkEquality(t, tokens, testCase.expected)
	}
